...
export const useListsStore = defineStore('lists', () => {
  ...
  const loadLists = async () => {
    ...
    try {
      ...
      for (const list of lists) {
        ...
        list.products = (productsRes.values ?? []).map((p: any) => ({
          ...p,
          checked: !!p.checked,
        }))
  ...
  const addProduct = async (
    ...
    try {
      ...
      await db.run(
        'INSERT INTO products (id, listId, name, quantity, checked) VALUES (?, ?, ?, ?, ?);',
        [id, listId, product.name, product.quantity, checkedValue],
      )
